...
import { DarkSelectField, DarkTextareaField } from './fields/DarkFormFields.jsx'
import { IconClose, IconSend, IconCheck } from './icons.jsx'

const TIPOS_REPORTE = ['Movimiento Diario', 'Zero Claims']

export default function ReporteDiarioModal({ onClose }) {
  ...
  return (
    <Modal onClose={onClose} className="bg-latam-estrellada text-white">
      ...
      <div className="px-6 pb-7 pt-6 sm:px-7">
        ...
        <h2 className="text-2xl font-extrabold leading-tight tracking-tight text-white">Formulario de Movimiento Diario</h2>
        <p className="mt-1 text-sm text-slate-300">Reporte de operación y Zero Claims</p>
        ...
        {sent ? (
          <div className="mt-6 flex items-center gap-2 rounded-xl bg-white/10 px-4 py-3.5 text-sm font-bold text-white animate-fadeUp">
            <IconCheck className="h-5 w-5 flex-shrink-0 text-green-400" /> Reporte enviado correctamente.
          </div>
        ) : (
          <form ref={formRef} noValidate onSubmit={submit} onInput={clearInvalid} onChange={clearInvalid} className="mt-6">
            <DarkSelectField
              ...
              invalid={invalidFields.has('tipoReporte')}
            />
            <DarkTextareaField
              name="detalle"
              ...
              invalid={invalidFields.has('detalle')}
            />
            ...
            <div className="mt-2 flex flex-col-reverse justify-end gap-3 border-t border-white/10 pt-5 sm:flex-row">
              ...
              >
                {sending ? 'Enviando...' : 'Enviar Reporte'} <IconSend className="h-4 w-4" />
              </button>
            </div>
          </form>
        )}
